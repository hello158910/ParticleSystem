...
void CParticle::setBehavior(int iType)
{
	...
	case EMITTER_DEFAULT:
		...
		break;
	case WATER:
		_Direction.x = rand() % 5 - 2.5f;
		_Direction.y = 1.5f + rand() %3;
		_fIntensity = 1;
		_fOpacity = 255;
		_fGravity = -5.0;
		_fSize = 1;
		...
		_fElapsedTime = 0;
		_fDelayTime = rand() % 100 / 1000.0f;
		_Particle->setScale((rand()%11 +5) /10.0f );
		break;
	case FIREWORK:
		_fVelocity = 6.5f;
		t = 2.0f * M_PI * (rand() % 1000) / 1000.0f;
		sint = sinf(t);  cost = cosf(t); cos4t = cosf(4 * t);
		_Direction.x =cost* cos4t;
		_Direction.y =cos4t*sint;
		_fLifeTime = 1.5f + LIFE_NOISE(0.15f);
		_fIntensity = 1;
		_fOpacity = 255;
		_fSpin = 0;
...
void CParticle::setPic(int iStyle) {
	switch (iStyle)
	{
	case COMET:
		/*for each (CParticle* p in _FreeList)
		{
		p->setPic("comet.png");
		}*/
		_Particle->initWithSpriteFrameName("comet.png");
		break;
	case SPARK:
		_Particle->initWithSpriteFrameName("spark.png");
		break;
	case BUBBLE:
		_Particle->initWithSpriteFrameName("bubble.png");
		break;
	case CIRCLE:
		_Particle->initWithSpriteFrameName("circle.png");
		break;
	case CLOUD:
		_Particle->initWithSpriteFrameName("cloud.png");
		break;
	case RAINDROP:
		_Particle->initWithSpriteFrameName("raindrop.png");
		break;
	case FLARE:
		_Particle->initWithSpriteFrameName("flare.png");
		break;
	}
}
